...
import com.example.kkaddak.api.dto.market.NFTThumbnailReqDto;
...
public class MemberController {
    ...
    @ApiResponses({
            @ApiResponse(code = 200, message = "나의 팔로워 목록 조회가 성공했을 경우 응답"),
            @ApiResponse(code = 404, message = "존재하지 않는 회원일 경우 응답")
    })
    @ApiOperation(value = "나의 팔로워 목록 조회 API")
    ...
    @ApiResponses({
            @ApiResponse(code = 200, message = "대표 nft 이미지 저장이 성공했을 경우 응답"),
            @ApiResponse(code = 404, message = "존재하지 않는 회원일 경우 응답")
    })
    @ApiOperation(value = "대표 nft 이미지 저장 API")
